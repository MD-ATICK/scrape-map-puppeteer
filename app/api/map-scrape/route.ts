...
import { parsePlacePreview } from "../test/route";
  ...
  const x = page.on("response", async (req) => {
    ...
    if (url.includes("/maps/preview/place")) {
      ...
      if (text.startsWith(")]}'")) {
        ...
        count++;
        if (!data.website) {
          scrapedData.push({ ...data , phone : phoneNumbers[0] as string | null  });
          return;
        }
        ...
        scrapedData.push({ ...data, email: emails[0], phone : phoneNumbers[0] as string | null});
      }
    }
  });
  return x;
